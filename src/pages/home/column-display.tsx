import { Card, Form, FormInput, Grid, Label } from "semantic-ui-react";
        ...
        {data.map((displayData:DisplayData) => (
            <Grid.Column key={displayData.id}>
                <Card.Group >
                    ...
                    <Card style={{height:800}} fluid image={`https://image.tmdb.org/t/p/original/${displayData.poster_path}`  } 
                    header={displayType === DisplayType.Movies ? displayData.title : displayData.name}
                    meta={`Release Data : ${displayData.release_date} | Vote Average : ${displayData.vote_average}`} 
                    description={displayData.overview.slice(0,350)+'...'}/>
                    {isRated && <Label> Your Rating is : {displayData.rating}</Label>}
                    </Link>
